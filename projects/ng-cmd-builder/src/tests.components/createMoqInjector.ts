...
export const resolveMock = <T>(token: Type<T> | InjectionToken<T>): IMock<T> => {
    const object = injector.get(token) as unknown;
    return object[MoqAPI];
};

export const resolve = <T>(token: Type<T> | InjectionToken<T> = testedToken): T => injector.get(token) as any;
